...
public interface FriendMapper extends BaseMapper<Friend> {
    @Select("select f.id, u.nickName from friend f, user u where f.uid=#{uid} and f.friend_id=u.id")
    List<FriendVo> getFriendList(Long uid);

    @Delete("delete from friend where uid=#{uid} and friend_id = #{friendId}")
    int deleteFriend(Friend friend);

    @Select("select f.id, u.nickName from friend f, user u where f.friend_id=u.id and u.name=#{name}")
